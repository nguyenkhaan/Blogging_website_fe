...
export default function BlogEditBoard() {
    ...
    return (
        <section className="w-full h-full bg-white md:shadow-xl/30 rounded-[5px] p-10 flex flex-col items-center text-black gap-10">
            ...
            {isEdit ? <div className="w-full flex items-center justify-end">
            <Link to = {`/dashboard?id=${getURLQuery(location).get('id')}`}>
                <motion.button
                    className="rounded-lg px-4 py-3 text-base cursor-pointer bg-blue-600 font-semibold text-white hover:bg-blue-700"
                    initial={{ scale: 1 }}
                    ...
                    onClick={() => {
                        ...
                        setTitleChange('') 
                        setEditorContent('')  //Dua ve cac gia tri mac dinh 
                        editorRef.commands.setContent('<p>Start writing your post...</p>') //Phai thong qua cai any 
                        setIsEdit(false) 
                    }}
                >
                    Write New Post
